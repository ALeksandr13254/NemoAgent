...
async def main():
    ...
    r = await run([status(451, ""), status(451, ""), ok("VPN")])
    results.append(check("451 (VPN off) is retried every 0.5 s and passes once the VPN is back", text_is(r, "VPN")
                         and 1.0 <= r["t"] < 1.4 and r["calls"] == 3, r))
    r = await run([status(451, "")], budget=0.8)
    results.append(check("a lasting 451 fails with its status in the message", isinstance(r["err"], nim.UpstreamError)
                         and r["err"].status == 451 and "451" in str(r["err"]) and "VPN" in str(r["err"]), r))
