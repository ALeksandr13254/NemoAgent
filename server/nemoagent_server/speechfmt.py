...
_FILLER_SENTENCES = [
    r"чем (?:ещё |еще )?(?:я )?(?:могу|смогу) (?:вам |тебе )?(?:помочь|быть полезен|быть полезна|быть полезным)",
    r"(?:чем|что) (?:ещё |еще )?(?:вам |тебе )?(?:помочь|подсказать)",
    r"(?:если|когда) (?:что-то|что-нибудь|что|нужно|понадобится|захотите|хотите|надо)[^.!?\n]{0,60}?(?:скажите|сообщите|дайте знать|обращайтесь|спрашивайте|напишите|пишите|говорите)[^.!?\n]{0,40}",
    r"(?:всегда )?(?:рад|рада|готов|готова|буду рад|буду рада) (?:помочь|быть полезным|быть полезной)[^.!?\n]{0,40}",
    r"обращайтесь(?: в любое время| если что| ещё| еще)?",
    r"(?:дайте знать|сообщите|скажите)[^.!?\n]{0,50}?(?:если|когда)[^.!?\n]{0,50}",
    r"(?:спрашивайте|задавайте вопросы)(?: ещё| еще)?",
    r"(?:надеюсь, )?(?:это|информация) (?:помогло|помогла|поможет|была полезна)[^.!?\n]{0,30}",
    r"how (?:else )?(?:can|may) i (?:help|assist)(?: you)?(?: today| further)?",
    r"(?:is there )?anything else(?: i can (?:help|do))?[^.!?\n]{0,30}",
    r"(?:just |please )?let me know if[^.!?\n]{0,60}",
    r"feel free to ask[^.!?\n]{0,40}",
    r"(?:happy|glad) to help[^.!?\n]{0,30}",
    r"hope (?:this|that|it) helps[^.!?\n]{0,30}",
]
