...
1. **Голосовой (диалоговый) агент** без инструментов. Отвечает пользователю сразу, в речевом формате: только
   слова, числа и единицы прописью в нужном падеже («двадцать четыре целых девять десятых гигабайта»),
   аббревиатуры раскрыты, названия транслитерированы («Виндоус», «Гитхаб»), короткие фразы, буква ё,
   `+` в омографах, без дежурных концовок. Если нужен код, команда, путь или ссылка, он говорит об этом
   словами, а точный текст ставит после строки `===` (только на экран). Если запрос требует действия или
   информации, которой у него нет (проверить, запустить, открыть, найти, посмотреть на экран, прочитать
   вложения, поискать в интернете, вспомнить), он говорит одну фразу о том, что сейчас сделает
   («Сейчас проверю место на диске»), и ставит задачу строкой `>>>` с точным описанием. Ответ стримится
   клиенту как речь по мере генерации (`speechfmt.ProseSpeechRouter`), первое предложение звучит примерно
   через секунду; последние семьдесят символов придерживаются, чтобы отрезать «Чем могу помочь?».
2. **Внутренний исполнитель** получает задачу и последние реплики диалога, выполняет её инструментами
   (команды, файлы, экран, вложения, веб-поиск, память) и возвращает фактический отчёт. С пользователем не
   разговаривает. Его вызовы и результаты видны в чате и в полном логе.
3. **Голосовой агент докладывает**: получает «Результат исполнителя» и озвучивает итог, цифры, ошибки.

Страховка от «сейчас проверю» без дела: если голосовой агент пообещал действие, но задачу не поставил,
сервер ставит её сам из запроса пользователя (в чате появляется пометка). За ход допускается до двух
задач подряд (например, проверить, а потом исправить).
...
промпт, история, вызовы инструментов и их результаты; каждый ответ с usage и временем).
...
режима, стиль текстового режима и промпт внутреннего исполнителя. «Сохранить» пишет их на сервер (`server/data/prompts.json`), они действуют со следующего сообщения
и переживают перезапуск; «Сбросить к умолчанию» возвращает встроенные тексты (`server/nemoagent_server/prompts.py`).
Ниже показан ровно тот промпт, который ушёл в модель в последнем запросе.
