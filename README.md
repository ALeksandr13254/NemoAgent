...
попали в речь, фраза обрезается до «команда на экране») и обход особенности рантайма, из-за которой один
ручной `+` отключал автоударения во всём предложении. Если в режиме `tool` модель ответила обычным текстом,
