...
- в каждый момент к пулу идёт один запрос;
...
- если сервер NIM недоступен вовсе (нет сети), попытки идут раз в `NIM_CONNECT_RETRY_DELAY_S` (0,5 с), чтобы
  отключение сети не превращалось в тысячи запросов в секунду;
...
Долгое ожидание в очереди последовательные повторы не сокращают. Для этого случая в `NIMClient` есть
необязательный параллельный режим, по умолчанию выключенный. В нём `NIM_PARALLEL` одинаковых запросов уходят
одновременно, выигрывает поток, первым выдавший проверенный фрагмент, остальные сразу закрываются; при
`NIM_HEDGE_AFTER_S` больше нуля один раз добавляется ещё один запрос, если самый старый из живых молчит это
время. Дополнительные запросы укладываются в поминутный бюджет `NIM_RPM_BUDGET` и прекращаются на
`NIM_RATE_LIMIT_COOLDOWN_S` после ответа 429. Сравнение режимов приведено в разделе 6.
...
Три режима повторов (раздел 3.5) сравнивались на одних и тех же вызовах. Задержка первого токена измерялась
от вызова `NIMClient.chat_stream` до первого проверенного фрагмента ответа, то есть с учётом всех повторов.
Режимы чередовались вызов за вызовом, по 5 вызовов на ячейку, с паузой 2 с между вызовами. В таблице
медиана (минимум–максимум); в последнем столбце среднее число HTTP-запросов на вызов в порядке столбцов.

| Вызов | Сразу, по одному (по умолчанию) | С паузами 0,3–3 с | Параллельно | Запросов на вызов |
|---|---|---|---|---|
| Короткий текстовый ответ | 0,72 с (0,43–0,83) | 0,45 с (0,40–18,92) | 1,08 с (0,62–2,05) | 1,4 / 2,8 / 2,4 |
| Вердикт маршрутизатора (JSON) | 0,81 с (0,47–10,95) | 1,29 с (0,43–17,20) | 2,04 с (2,00–3,38) | 2,2 / 2,6 / 3,0 |
| Исполнитель: вызов инструмента | 1,19 с (0,76–2,36) | 2,14 с (1,24–4,10) | 1,78 с (0,80–2,32) | 2,0 / 2,6 / 1,8 |
| Скриншот 1600×900 с вопросом | 1,68 с (0,99–2,11) | 2,93 с (0,83–6,95) | 0,92 с (0,72–1,73) | 2,0 / 2,8 / 1,2 |

| Режим | Медиана | 90-й процентиль | Максимум | Ответов 429 |
|---|---|---|---|---|
| Сразу, по одному (по умолчанию) | 1,02 с | 2,11 с | 10,95 с | 2 |
| С паузами 0,3–3 с | 1,77 с | 11,29 с | 18,92 с | 5 |
| Параллельно | 1,60 с | 2,18 с | 3,38 с | 0 |

Мгновенный последовательный повтор дал наименьшую общую медиану и почти тот же 90-й процентиль, что и
параллельные запросы, при сопоставимом числе запросов на вызов; паузы между повторами растягивают хвост
распределения.
Худший вызов этого режима (10,95 с) пришёлся на два ответа 429 подряд, после которых сервер выдерживал паузу.
...
Выбор модели и схемы запросов опирается на наблюдения того же вечера. Nemotron 3.5 Lightning 30B на том же пуле
выдавала первый токен с медианой 29 с (от 18,6 до 93,5 с, 6 запросов), поэтому все роли по умолчанию переведены на
Omni. Первая версия параллельных запросов не учитывала поминутный лимит ключа: при плотной нагрузке шлюз начинал
отвечать 429, и отдельные вызовы разрастались до 17 запросов и 28 с. После добавления бюджета запросов
параллельный режим стал работоспособным, но по сравнению выше уступил мгновенному последовательному повтору,
который и выбран по умолчанию.
...
- `NIM_PARALLEL`, `NIM_MAX_PARALLEL`, `NIM_HEDGE_AFTER_S`, `NIM_RPM_BUDGET`, `NIM_RATE_LIMIT_COOLDOWN_S`:
  необязательный параллельный режим, по умолчанию выключен (`NIM_PARALLEL=1`, `NIM_HEDGE_AFTER_S=0`);
...
- `nim.py`: `NIMClient`: потоковые вызовы chat/completions с инструментами, мгновенный повтор отказавшего
  ...
  429 пауза по `Retry-After`), необязательный параллельный режим с бюджетом запросов ключа, детектор
  зацикливания генерации, отключение режима рассуждений через `chat_template_kwargs.enable_thinking`,
  эмбеддинги.
...
- `server/tests/test_nim_race.py`: проверка повторов на имитации NIM (27 сценариев: мгновенный повтор после 503,
  пустого потока и ошибки в тексте; ожидание после 429 по `Retry-After` и по нарастающим паузам, общее для всех
  вызовов сервера и не расходующее бюджет повторов; недоступная сеть, фатальные ошибки, прерывание, а также
  необязательный параллельный режим); сеть и ключ не нужны.
