...
Голосовой и текстовый агент из двух частей: **сервер** (мозг: `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`
через NVIDIA NIM — одна модель, которая принимает текст, картинки, аудио и видео и умеет function calling;
долговременная память на эмбеддингах NIM) и **клиент** на машине пользователя (микрофон → faster-whisper,
TeraTTSv2 → колонки, локальный веб-интерфейс, исполнение команд и управление компьютером).
...
```
 пользователь ──голос/текст/файлы/скриншоты──▶ клиент (client/)                сервер (server/)
   ...
   колонки ◀─ TeraTTSv2 ◀─ предложения ◀──дельты──◀ ws ◀── nemotron-3-nano-omni (text + image_url/audio_url/video_url, tools)
...
| первый токен Nemotron 3 Nano Omni (без reasoning) | 2–2.6 с |
...
- Модель официально «English-first», но по-русски отвечает и транскрибирует русскую речь (проверено);
  reasoning выключен по умолчанию (`LLM_THINKING=false`) ради задержки, включается в `.env` (тогда
  `LLM_TEMPERATURE=0.6`, как советует карточка модели).
