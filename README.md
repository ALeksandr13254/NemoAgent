...
- `prompts.py` — промпты обоих агентов, маршрутизатора и правила речи; переопределения приходят от клиента
  и живут только в его сессии.
- `attachments.py` — вложения в оперативной памяти со сроком жизни `UPLOAD_TTL_S` (2 часа без обращений);
  на диск сервер не пишет ничего.

**Сервер без состояния.** Один сервер обслуживает много клиентов параллельно (одна сессия на соединение) и
прячет вызовы NIM; всё, что должно пережить перезапуск, хранится у клиента в `client/data`: история чатов
(`chats/*.json`), оригиналы вложений (`attachments/`, файл удаляется вместе со своим чатом, а не попавшие
ни в один чат — через `ATTACHMENT_KEEP_HOURS`), долговременная память (`memory.sqlite3`), настройки
(`settings.json`) и правки промптов (`prompts.json`). Серверу от клиента приходят: вложения на время хода
(`POST /upload`, в RAM), найденные воспоминания вместе с сообщением (`memory_context`), правки промптов при
подключении; сервер отдаёт эмбеддинги через `POST /embed` и после каждого хода присылает `memo` — то, что
клиент кладёт в память. Инструмент `search_memory` исполнителя тоже обращается к клиенту.
...
- `gui_action` работает по координатам из `look_at_screen` (кадр скриншота); если `SCREENSHOT_MAX_SIDE`
  ...
  в кадре которого заданы координаты.
- **Мониторы для скриншота** выбираются в настройках клиента: все (по умолчанию), один или любой набор;
  и кнопка 🖥, и `look_at_screen` снимают по одной картинке на каждый выбранный монитор. Начальное
  значение — `SCREENSHOT_MONITORS` в `client/.env`.
