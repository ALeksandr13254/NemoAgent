...
3. [Обработка запроса](#3-обработка-запроса)
   ...
   - [3.6. Точный подсчёт токенов](#36-точный-подсчёт-токенов)
...
- **Сервер** (`server/`, Python, FastAPI). Принимает соединения клиентов по WebSocket, ведёт контекст
  ...
  вложений, веб-поиск, чтение страниц). Все роли работают на мультимодальной модели
  `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`, которая принимает текст, изображения, аудио и видео, поэтому
  каждая роль сама видит вложения и скриншоты; для любой роли можно выбрать и чисто текстовую
  `nvidia/nemotron-3.5-lightning-30b-a3b`. Отказавший запрос к пулу NIM
...
flowchart LR
    ...
    subgraph NIM["NVIDIA NIM (integrate.api.nvidia.com)"]
        ...
        M2["Nemotron 3.5 Lightning 30B<br/>(по выбору, только текст)"]
...
одной мультимодальной модели (Nano Omni), каждая со своим промптом (все промпты редактируются во вкладке
«Системные промпты»). Отдельной модели для медиа нет: каждая роль сама видит изображения и скриншоты, слышит
аудио и смотрит видео. Если для роли выбрана текстовая Lightning, в её вызовах медиа заменяются короткой
текстовой пометкой, и вложений она не видит.
...
2. **Маршрутизатор** работает параллельно с ответом диалогового агента. Он получает текст запроса как данные
   ...
   доступными агенту, поэтому вопросы об их содержимом («что ты видишь», «что на картинке», «прочитай»)
   исполнителя не требуют. Кроме того, если в сообщении есть вложения и диалоговый агент ответил по существу,
   вердикт маршрутизатора не применяется: агент видел файлы сам, а маршрутизатор читал только текст запроса.
...
3. **Исполнитель** получает задачу, последние реплики диалога и вложения текущего сообщения, выполняет
   ...
   логе. Вложения текущего сообщения уже лежат в его задаче, скриншоты `look_at_screen` приходят ему
   изображениями сразу после результата инструмента; `view_attachments` нужен только для файлов из более
   ранних сообщений, медиа которых уже убраны из контекста.
...
### 3.6. Точный подсчёт токенов

Сервер знает размер запроса в токенах до отправки и без обращения к NVIDIA: он повторяет то, что делает облачная
модель (vLLM за `integrate.api.nvidia.com`), по файлам самой модели. Модуль `tokencount.py` при первом запуске
скачивает из репозитория `nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-BF16` на Hugging Face три файла (около
17 МБ) в `server/models/omni-tokenizer/` и затем:

1. собирает текст запроса шаблоном `chat_template.jinja` в том же окружении Jinja, что и transformers;
   аргументы вызовов инструментов передаются шаблону объектами, `enable_thinking` берётся из запроса;
2. токенизирует текст по `tokenizer.json` (библиотека tokenizers, без BOS);
3. добавляет токены вложений. Изображение даёт `<img>`, N токенов `<image>` и `</img>`, где N вычисляется по
   формуле процессора модели: патчи 16 px, от 1024 до 13 312 патчей на изображение, один токен на 4 патча.
   Аудио даёт `<so_start>`, N токенов и `<so_end>`, где N зависит от длины записи (кадр 10 мс, затем три свёртки
   с шагом 2), а длина определяется декодированием записи через ffmpeg;
4. видео облачный сервер нарезает на кадры не так, как опубликованный код, поэтому каждый ролик измеряется
   один раз: сервер отправляет запрос только с этим роликом и просьбой ответить одним словом и вычитает из
   `prompt_tokens` ответа всё остальное. Замер занимает около двух секунд. Если ролик ушёл в модель до замера,
   его размер берётся из `prompt_tokens` этого ответа, и отдельный запрос не нужен. Первый фрагмент потока с
   `continuous_usage_stats` для замера не подходит: в нём видео учтено как текстовая метка, а итоговое число
   приходит в конце потока.

Результат сравнивался с `prompt_tokens`, которые возвращает NVIDIA:

| Запрос | Счётчик | NVIDIA |
|---|---|---|
| Русская фраза | 52 | 52 |
| Английская фраза | 40 | 40 |
| Изображение 1600×900 с вопросом | 1476 | 1476 |
| Изображение 800×450 с вопросом | 401 | 401 |
| Изображение 400×225 с вопросом | 312 | 312 |
| Аудио 7,3 с с вопросом | 131 | 131 |
| Диалог: системный промпт, история, воспоминания, два изображения | 3701 | 3701 |
| Исполнитель: инструменты, вызовы, результаты, скриншот | 4954 | 4954 |
| Диалог с изображением 1280×720 и видео 6 с (видео измерено отдельно) | 3345 | 3345 |
| Запросы из 1 000 000 и 1 000 001 токенов (число из текста ошибки) | 1 000 000 и 1 000 001 | 1 000 000 и 1 000 001 |

В рабочем прогоне через интерфейс совпали все 20 запросов хода: маршрутизатор, диалоговый агент и исполнитель
с инструментами и их результатами.

Облачная модель отклоняет запрос уже из 1 000 000 токенов (ответ 400: «This model's maximum context length is
1000000 tokens. However, your messages resulted in 1000000 tokens»), поэтому в запрос помещается не больше
999 999 токенов. На ответ остаётся разность между лимитом и размером запроса: параметр `max_tokens` облачная Omni
не соблюдает (при `max_tokens=5` она вернула 1212 токенов).

Подсчёт быстрый. Текст запроса режется по токену `<|im_start|>`, с которого начинается каждое сообщение. Это
специальный токен, и текст по разные его стороны никогда не склеивается в один токен, поэтому сообщения
токенизируются независимо, а посчитанные запоминаются: при повторном подсчёте того же чата заново считается
только изменившееся сообщение. Длинное сообщение дополнительно режется перед пробелом, стоящим между двумя
буквами (предтокенизатор модели всегда заканчивает кусок на такой границе), и куски токенизируются
параллельно. Сумма совпадает с токенизацией целого текста; это проверено на русском, английском и смешанном
тексте, коде, тексте без пробелов и тексте с переносами строк. Обычный чат считается за 1–15 мс; 576 тысяч
токенов при первом подсчёте занимают 0,22 с против 1,36 с у токенизации целиком, при повторном 0,014 с; запрос
из 1,05 млн токенов вместе с расчётом допустимой длины сообщения обрабатывается примерно за 2 с.

Применение:

- под полем ввода чата выводится точный размер запроса, который уйдёт при отправке: системный промпт,
  история после автоматической обрезки, сообщение с вложениями (сообщения протокола `count_tokens` и
  `token_count`). Подсказка при наведении раскладывает число по частям и показывает остаток на ответ. Число
  обычного цвета, пока запрос помещается; жёлтое, если при отправке старые ходы уйдут из контекста
  (`CONTEXT_BUDGET_TOKENS`); красное при превышении лимита модели. Тогда указано, на сколько токенов запрос
  больше допустимого, до скольких символов сократить сообщение (длина проверяется повторным подсчётом
  укороченного запроса) и сколько самых старых сообщений удалить, если этого достаточно;
- воспоминания из памяти подбираются при отправке и в это число не входят; при включённой памяти подсказка
  об этом предупреждает;
- автоматическая обрезка контекста использует точные числа, а не оценку по длине текста;
- каждый запрос к модели считается параллельно с отправкой, и число сверяется с `prompt_tokens` ответа:
  расхождение пишется в журнал сервера предупреждением, во вкладке «Полные логи» у ответа стоит отметка
  «счётчик: совпал ✓», а в строке метрик после ответа показан размер запроса;
- во вкладке «Системные промпты» у каждого поля показано число его токенов.

Счётчик рассчитан на Nano Omni. Для текстовой Lightning с другим токенизатором под полем ввода выводится
пометка вместо числа, а обрезка контекста идёт по прежней оценке.
...
текущему диалогу. Живой контекст удерживается в пределах `CONTEXT_BUDGET_TOKENS` (60 000 токенов, размер
запроса считается точно, раздел 3.6): старые ходы уходят из контекста, но остаются в памяти; медиа старше `MEDIA_KEEP_TURNS` (двух) ходов заменяются
текстовой пометкой.
...
- `LLM_MODEL`: модель по умолчанию для диалогового агента и исполнителя (Nano Omni); `ROUTER_MODEL`: модель
  маршрутизатора (та же). Клиент переопределяет каждую роль из своих настроек;
- `TEXT_ONLY_MODELS`: модели без поддержки медиа (по умолчанию Lightning); в их вызовах изображения, аудио и
  видео заменяются текстовой пометкой;
...
протокол описан в `server/nemoagent_server/main.py`. При первом запуске сервер скачивает файлы токенизатора
модели (около 17 МБ, раздел 3.6); до окончания загрузки он работает, но счётчик токенов недоступен.
...
`MODEL_ROUTER`), языки распознавания и озвучки, голоса, параметры детектора речи, режим
...
- **Чат**: диалог с агентом. В поле ввода: отправка по Enter, кнопки вложений, скриншота, памяти,
  ...
  уходит уже с учётом изменений. Если в этот момент генерируется ответ, он прерывается. Под полем ввода
  показан точный размер запроса, который уйдёт при отправке, и остаток до лимита модели (раздел 3.6).
- **Системные промпты**: пять редактируемых полей (шаблон промпта диалогового агента с подстановками
  ...
  встроенные тексты. У каждого поля показано число его токенов. Ниже отображается промпт, ушедший в модель
  в последнем запросе.
...
умолчанию или текстовая Nemotron 3.5 Lightning 30B), режим озвучки, язык озвучки («авто» по тексту
...
- `agent.py`: `AgentSession`: системный промпт с описанием окружения клиента, три роли модели, модель по роли
  (для текстовой модели медиа заменяются пометкой), обрезка контекста по точному размеру, размер следующего
  запроса для поля ввода, сверка каждого запроса с `prompt_tokens`, воспоминания от клиента, прерывание,
  событие `done` с данными для памяти клиента.
- `tokencount.py`: точный подсчёт токенов запроса к Nano Omni по файлам модели: шаблон, токенизатор,
  формулы изображений и аудио, замеры видео, кэш сообщений и параллельная токенизация (раздел 3.6).
...
- Ограничения Omni: контекст 1 000 000 токенов (в запрос помещается 999 999), видео до 2 минут (сервер
  обрезает и сжимает), аудио до часа (сервер обрезает до `MEDIA_AUDIO_MAX_S`, по умолчанию 20 минут).
  Изображение 1600×900 занимает 1452 токена, 90 с речи 1128 токенов, видео 6 с 640×360 1069 токенов; точный
  размер любого запроса показывает счётчик под полем ввода. Запрос объёмом 19 МБ проходит;
  `MEDIA_MAX_INLINE_MB` (24) ограничивает размер одного файла.
- Облачная Omni не соблюдает `max_tokens`, поэтому `DIALOGUE_MAX_TOKENS` и `LLM_MAX_TOKENS` длину её ответа
  не ограничивают: она задаётся промптом, а предел ей ставит только остаток контекста.
- Lightning работает только с текстом (запрос с изображением к ней шлюз отклоняет кодом 400), поэтому в её
  вызовах изображения, аудио и видео заменяются текстовой пометкой: выбранная для роли Lightning вложений и
  скриншотов не видит. Lightning изредка зацикливается, повторяя один фрагмент до
