...
Когда клиент хочет озвучку, модель получает набор правил речевого формата: только слова, числа и единицы
прописью в нужном падеже («двадцать четыре целых девять десятых гигабайта», «пятнадцать ноль две»,
«восемьдесят процентов»), аббревиатуры раскрыты, иностранные названия в русской речи транслитерированы
(«Виндоус», «Гитхаб»), короткие фразы без списков, буква ё на месте, `+` перед ударной гласной только в
омографах («з+амок / зам+ок»), без дежурных концовок. Правила выведены из карточки модели и её таблицы
символов `unicode_indexer.json`. Есть два режима (`SPEECH_MODE` в `server/.env`):

- **`prose`** (по умолчанию): модель пишет сам ответ в речевом формате, он стримится клиенту по мере
  генерации, и первое предложение звучит примерно через секунду. Если нужно показать код, команду, путь,
  ссылку или таблицу, модель сначала говорит об этом словами, а точный текст ставит после строки `===`:
  эта часть показывается на экране и не озвучивается.
- **`tool`**: модель вызывает инструмент `speak(speech, display)`. NIM отдаёт аргументы вызова одним
  куском в конце, поэтому голос стартует только после полной генерации; кроме того, модель воспринимает
  вызов как «действие» и норовит добавить «Чем могу помочь?». Режим оставлен как опция.

В обоих режимах сервер отрезает дежурные концовки («Чем могу помочь?», «Если нужно что-то ещё, скажите»,
«Let me know…») до того, как они уйдут в озвучку, историю и память: последние семьдесят символов
придерживаются, пока ответ не закончится. На клиенте дополнительно работает фильтр по словарю (тире и
многоточия превращаются в паузы, `%`/`№`/`°` — в слова), скруббер кода (если команда, путь или ссылка всё же
попали в речь, фраза обрезается до «команда на экране»), обход особенности рантайма, из-за которой один
ручной `+` отключал автоударения во всём предложении, и транслитерация (`translit.py`): латинские слова в
русском предложении переписываются кириллицей («futuristic» → «футуристик», «Visual Studio Code» →
«Визуал Студио Код», «GTA» → «джи-ти-эй») по словарю частых терминов и правилам чтения, потому что русский
голос читает `<en>`-вставки как шум. Целиком английские фразы озвучивает английский голос. Если в режиме `tool` модель ответила обычным текстом,
сервер переписывает его в речевой формат отдельным запросом (`SPEAK_REWRITE_MODEL`). `tool_choice=required`
намеренно не используется: в потоковом режиме NIM тогда иногда выводит JSON вызова как текст до лимита токенов.
