...
3. [Обработка запроса](#3-обработка-запроса)
   ...
   - [3.5. Параллельные запросы к NIM](#35-параллельные-запросы-к-nim)
...
- **Сервер** (`server/`, Python, FastAPI). Принимает соединения клиентов по WebSocket, ведёт контекст
  ...
  текстовых ролей можно выбрать `nvidia/nemotron-3.5-lightning-30b-a3b`. К пулу NIM сервер обращается
  параллельными запросами (раздел 3.5). Сервер не пишет на диск и обслуживает произвольное число клиентов
  одновременно.
...
### 3.5. Параллельные запросы к NIM

Бесплатный пул NIM отказывает тремя способами: немедленным кодом 503, когда заняты все слоты воркера;
принятым запросом, который десятки секунд стоит в очереди; потоком, который закрывается без единого токена.
Последовательные повторы помогают только в первом случае и добавляют паузу к каждой попытке. Поэтому
`NIMClient` отправляет одинаковые запросы параллельно и оставляет тот, что ответил первым (рис. 6):

- одновременно уходят `NIM_PARALLEL` (два) запроса;
- побеждает поток, первым выдавший проверенный результат: фрагмент текста после проверки на ошибку, переданную
  шлюзом в виде текста, фрагмент рассуждения или вызов инструмента. Остальные потоки сразу закрываются, что
  освобождает их слоты в пуле. До выбора победителя клиенту ничего не передаётся, поэтому неудачные попытки
  пользователю не видны;
- отказавший запрос заменяется новым через 0,3; 0,6; 1,0; 1,5; 2,0; 2,5 или 3 с (последняя пауза
  повторяется), пока не исчерпан бюджет `NIM_RETRY_BUDGET_S` (30 с);
- если самый старый из живых запросов не дал результата за `NIM_HEDGE_AFTER_S` (6 с), один раз за вызов
  добавляется ещё один запрос, но не более `NIM_MAX_PARALLEL` (трёх) одновременно;
- у ключа есть поминутный лимит, общий для всех сессий сервера. Дополнительные запросы отправляются, только
  пока за последнюю минуту их ушло меньше `NIM_RPM_BUDGET` (30). После ответа 429 параллельность отключается
  для всех сессий на `NIM_RATE_LIMIT_COOLDOWN_S` (60 с), а пауза перед повтором берётся из заголовка
  `Retry-After` (без него 5 с);
- ошибка, возникшая после того, как текст уже ушёл клиенту, молча не повторяется, чтобы ответ не задвоился.
...
sequenceDiagram
    ...
    participant A as NIM, запрос A
    participant B as NIM, запрос B
    participant C as NIM, запрос C

    S->>N: chat_stream(сообщения, инструменты)
    par одновременно
        N->>A: POST chat/completions, поток
    and
        N->>B: POST chat/completions, поток
    end
    A-->>N: 503 или пустой поток
    Note over N: замена через 0,3 с
    N->>C: POST chat/completions, поток
    B-->>N: первый проверенный фрагмент
    Note over N: B выигрывает, C закрывается
    N-xC: соединение закрыто
    N-->>S: дельты ответа B, затем Completion
```

*Рис. 6. Диаграмма последовательности параллельных запросов к NIM.*
...
умолчанию два часа без обращений). Всё, что должно пережить перезапуск, хранится на клиенте (рис. 7).
...
Nano Omni во всех ролях, режим рассуждений выключен, 25.09.2026, вечер по Москве. Задержка первого токена
измерялась от вызова `NIMClient.chat_stream` до первого проверенного фрагмента ответа, то есть с учётом всех
повторов. Режимы чередовались вызов за вызовом, по 6 вызовов на ячейку, с паузой 2 с между вызовами, так
что оба режима работали в одних и тех же условиях пула.
...
Первый токен Nano Omni, медиана (минимум–максимум); в последнем столбце среднее число HTTP-запросов на один
вызов в режимах «один запрос» и «параллельно»:

| Вызов | Один запрос | Параллельные запросы | Запросов на вызов |
|---|---|---|---|
| Короткий текстовый ответ (роль диалогового агента) | 0,81 с (0,43–5,49) | 0,45 с (0,40–1,13) | 1,5 / 1,8 |
| Вердикт маршрутизатора, строка JSON | 1,28 с (0,49–3,34) | 0,46 с (0,43–17,83) | 1,8 / 2,8 |
| Исполнитель: вызов инструмента | 1,54 с (0,80–2,87) | 1,23 с (0,71–1,28) | 1,7 / 1,8 |
| Скриншот 1600×900 (около 1600 токенов) с вопросом | 2,58 с (1,10–4,29) | 1,39 с (0,83–5,11) | 2,2 / 2,5 |

По всем вызовам медиана первого токена составила 1,23 с для одиночных запросов и 0,89 с для
параллельных, 90-й процентиль 3,53 и 3,43 с соответственно; ответов 429 за прогон: 1. Максимум 17,83 с у
маршрутизатора пришёлся на полную перегрузку пула: восемь ответов 503 подряд при очереди воркера до «384/16».
Когда отказывают все запросы, параллельность не помогает, и время определяется паузами между повторами.
Суммарная задержка от конца фразы пользователя до первого звука ответа складывается из распознавания, первого
токена и синтеза первого предложения и в типичном случае составляет около двух секунд.

Выбор модели и схемы запросов опирается на два наблюдения того же вечера. Nemotron 3.5 Lightning 30B на том же
пуле выдавала первый токен с медианой 29 с (от 18,6 до 93,5 с, 6 запросов), поэтому все роли по умолчанию
переведены на Omni. Первая версия параллельных запросов не учитывала поминутный лимит ключа: при плотной нагрузке
шлюз начинал отвечать 429, и отдельные вызовы разрастались до 17 запросов и 28 с. Отсюда бюджет запросов и
пауза после 429, описанные в разделе 3.5.
...
- `LLM_MODEL`: модель по умолчанию для диалогового агента и исполнителя (Nano Omni); `ROUTER_MODEL`: модель
  ...
  модель роли;
- `NIM_PARALLEL`, `NIM_MAX_PARALLEL`, `NIM_HEDGE_AFTER_S`, `NIM_RPM_BUDGET`, `NIM_RATE_LIMIT_COOLDOWN_S`,
  `NIM_RETRY_BUDGET_S`: параметры параллельных запросов и повторов (раздел 3.5); `NIM_PARALLEL=1` вместе с
  `NIM_HEDGE_AFTER_S=0` возвращают отправку по одному запросу;
...
- `nim.py`: `NIMClient`: потоковые вызовы chat/completions с инструментами, параллельные запросы с выбором
  первого ответившего потока и бюджетом запросов ключа, повтор при перегрузке пула (коды 429, 500, 503, 529,
  текст ошибки в теле ответа, пустой поток), детектор зацикливания генерации, отключение режима рассуждений
  через `chat_template_kwargs.enable_thinking`, эмбеддинги.
...
- `server/tests/test_nim_race.py`: проверка параллельных запросов на имитации NIM (15 сценариев: выбор
  первого потока, замена отказавших, запасной запрос, 429 и бюджет запросов, прерывание); сеть и ключ не нужны.
...
- Бесплатный пул NIM общий и ограниченный (у Omni 16 одновременных запросов на воркер): в часы пик шлюз
  ...
  изображению может прийти через десятки секунд; параллельные запросы (раздел 3.5) сокращают это ожидание.
- У ключа NIM есть поминутный лимит запросов, при превышении шлюз отвечает кодом 429. Параллельные запросы
  расходуют лимит быстрее, поэтому сервер следит за бюджетом `NIM_RPM_BUDGET` и после ответа 429 на минуту
  переходит на одиночные запросы.
