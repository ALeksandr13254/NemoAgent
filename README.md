...
- `server/tests/test_nim_retries.py`: проверка повторов на имитации NIM (17 сценариев: мгновенный повтор после
  503, пустого потока и ошибки в тексте; ожидание после 429 по `Retry-After` и по нарастающим паузам, общее для
  всех вызовов сервера и не расходующее бюджет повторов; недоступная сеть, фатальные ошибки, ошибка после начала
  ответа, прерывание); сеть и ключ не нужны.
