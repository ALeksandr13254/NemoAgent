...
- **Сервер** (`server/`, Python, FastAPI). Принимает соединения клиентов по WebSocket, ведёт контекст
  ...
  вложений, веб-поиск, чтение страниц). Для всех текстовых вызовов используется модель
  `nvidia/nemotron-3-super-120b-a12b`; вызовы, в которых присутствуют изображения, аудио или видео,
  направляются в мультимодальную модель `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`. Сервер не пишет
...
flowchart LR
    ...
    subgraph NIM["NVIDIA NIM (integrate.api.nvidia.com)"]
        direction TB
        M1["Nemotron 3 Super 120B (текст)"]
        M2["Nemotron 3 Nano Omni 30B (медиа)"]
...
Основная особенность бесплатного пула состоит не в задержке, а в пустых потоках: шлюз отвечает кодом 200 и
закрывает поток, не передав ни одного токена. В приведённом прогоне так завершились 2 запроса из 16 к Super
и 6 из 11 к Omni. Сервер распознаёт пустой поток и повторяет запрос, поэтому для пользователя это выглядит
как дополнительные две-три секунды ожидания, а не как ошибка.

Модель для текстовых ролей (диалоговый агент, исполнитель, маршрутизатор) можно переключить в настройках на
`nvidia/nemotron-3.5-lightning-30b-a3b`; измерения для неё не проводились, по умолчанию она не используется.
...
- `LLM_MODEL`: модель по умолчанию для диалогового агента и исполнителя (Nemotron 3 Super 120B);
  `ROUTER_MODEL`: модель маршрутизатора (та же); `LLM_MEDIA_MODEL`: модель для вызовов с изображениями,
  аудио и видео (Nano Omni). Клиент переопределяет каждую роль из своих настроек. Модель выбирается на
  каждый вызов: пока в контексте есть медиа последних двух ходов или скриншот исполнителя, используется
  мультимодальная модель, иначе модель роли;
...
Настройки (правая колонка): модель на каждую роль (диалоговый агент, исполнитель, маршрутизатор: Super 120B
по умолчанию или Lightning 30B; медиа-роль: Nano Omni), режим озвучки, язык озвучки («авто» по тексту
...
- `nim.py`: `NIMClient`: потоковые вызовы chat/completions с инструментами, повтор при перегрузке пула
  (коды 500, 503, 529, текст ошибки в теле ответа, пустой поток), детектор зацикливания генерации,
  отключение режима рассуждений через `chat_template_kwargs.enable_thinking`, эмбеддинги.
...
- Обе модели заявлены как ориентированные на английский язык, однако отвечают по-русски, а Omni
  ...
  задержки; при включении рекомендуется `LLM_TEMPERATURE=0.6` согласно карточке модели.
- Бесплатный пул NIM общий и ограниченный (у Omni 16 одновременных запросов на воркер): в часы пик шлюз
  ...
  изображению может прийти через десятки секунд. Пустые потоки у Omni наблюдаются заметно чаще, чем у
  Super (раздел 6).
...
- Super 120B работает только с текстом: любой вызов с медиа выполняется Omni, а устаревшие медиа заменяются
  пометкой, чтобы текстовые ходы не переносили изображения.
