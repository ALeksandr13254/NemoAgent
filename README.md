...
Во вкладке «Системный промпт» три редактируемых поля: шаблон промпта (подстановки `{env}` — окружение и
текущее время, `{voice_style}` — стиль по режиму), правила речи для голосового режима и стиль текстового
режима. «Сохранить» пишет их на сервер (`server/data/prompts.json`), они действуют со следующего сообщения
